...
function set_player(player, team, mute)
  ...
  local character_name = old_character and old_character.name or "player"
  player.character = nil
  if old_character then old_character.die() end
  ...
  player.set_controller
  {
    ...
    character = surface.create_entity{name = character_name, position = position, force = force}
  }
...
function update_kill_score_frame(player)
  ...
  local scores = script_data.kill_scores
  local rank = 1
...
function update_space_race_frame(player)
  ...
  for name, score in spairs (script_data.rocket_scores, function(t, a, b) return t[b] < t[a] end) do
    if team_map[name] then
      local position = information_table.add{type = "label", caption = "#"..rank}
      if name == player.force.name then
        position.style.font = "default-semibold"
...
function check_update_space_race_score()
  ...
  local scores = script_data.rocket_scores
  for k, player in pairs (game.connected_players) do
    update_space_race_frame(player)
  end
...
local on_rocket_launched = function(event)
  if not script_data.config.victory.space_race then return end
  local sent = script_data.rocket_scores
